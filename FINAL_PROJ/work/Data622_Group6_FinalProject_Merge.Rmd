...
Lastly, we visualize each year's crime level geographically below.  We can see the clear presence of high crime levels in a small number of subdivisions that persist across the 3 years while most subdivisions are low crime.
...
We observe a low but significant spatial dependency between crime levels in adjoining residential subdivisions.
The spatial lag regression is statistically significant as its p-value $p=1.31\%$.  The spatial autoregressive coefficient $\rho = 0.10$ appears small in magnitude but this may be explained by the geographic dispersion of the subdivisions.   Some subdivisions are geographically isolated from others.   Adjacent subdivisions are often separated by landscaping and trees or lack connecting paths or roads.  

Now let us interpret the coefficients.

  * The past crime level `2018` is very statistically significant and positively associated with `2019` crime level.   45% of last year's crime level contributes to our predicted `2019` level.

  * `approvedlots` is very significant and positively associated with `2019` crime.  An extra 100 lots predicts an increase of 1.07 crime incidents per year.   This makes sense.  More residents means more crimes.
  
  *  `avg_income` is highly significant and negatively related to crime levels.   A \$100,000 increase in median household income (`avg_income`) predicts 0.86 fewer incidents per year *after* incorporating the past year's crime level.  This is a large impact because the median number of incidents per year is 1.
  
  * `shape_starea` the area in the subdivision is not statistically significant and the coefficient is nearly zero. Perhaps, crime rarely occurs on empty land but requires the inhabitants as perpetrators and victims.

  * The residential subdivision `category` is a factor variable.  Its base level (`Apartment`) is associated with 2.95 incidents per year (all dummies are set to zero).   `category`=`Mobile Home` adds 1.27 incidents over the base level.  The most frequent category is `Single-Family Detached` which subtracts 2.03 incidents from the base level.   These are large effects and most coefficients are statistically significant.  `Mixed Residential` is not significant because of the small sample size (only 4).

Comparing the OLS to the spatial lag model, the OLS coefficients are larger.   For example, \$100,000 increase in `avg_income` reduces crime by 1.03 incident per year in the OLS model vs. 0.86 in the spatial lag model.  This difference is expected because the spatial lag model allocates some of the observed response to spatial effects.   Because crime level in Cary is quite low (the median number of crimes is 1 incident per year), small changes in parameter values are actually quite significant as a percentage of crime levels.
...
Spatial regression is a fruitful method to investigate crime incidents in residential subdivisions.    We experimented with non-traditional spatial weight measures to overcome technical challenges to conducting spatial regressions and produce intuitively sensible model coefficients.   We find that past crime levels, household income, the category of residence and number of homes in the subdivision affect predicted crime levels.   A next logical step is to backtest such a regression model with subsequent periods.   In addition, we conjecture that spatial dependency in the Town is local rather than global.  A future direction of study is to use localized spatial regression analysis to analyze crime in such a sprawling town.   Finally, residential subdivisions data need to be paired with commercial districts to create a holistic picture of municipal crime.   After all, the first law of geography does not care about residential zoning regulations.
