...
lap_data_split <- initial_split(cla, prop=0.80, strata = Loan_Status)

lap_training <- lap_data_split %>% training()

lap_test <- lap_data_split %>% testing()

lap_folds <- vfold_cv(lap_training, v=5, repeats=5)

# Specify the model
mod_boost <- boost_tree(trees = 10, # 1000 at tune usage URL
                        learn_rate = tune(),
                        sample_size = tune(),
                        mtry = tune(),
                        min_n = tune(),
...
boost_params <- parameters(
  ...
  tree_depth(),
  learn_rate(),
  finalize(mtry(), select(lap_training,-Loan_Status)),
