...
```{r knn-5-decision-boundary, eval=F}
...
```{r knn-1-decision-boundary, eval=F}
...
```{r, echo=FALSE,results = FALSE, message=FALSE, eval=F }
set.seed(10)
...
```{r fig.cap="'Model' Using only Credit History" , eval=F}
...
Gradient Boosting model approach is an ensemble method that applies repeated models to resampled data and pooling estimates. A boosted tree fits the next tree to the residuals of the prior tree. The residual error is minimized through each successive tree and typically accomplished through shallow trees.

## Feature selection

Based on the importance factor of the features from an earlier Decision Tree model, the following variables are selected as input into the gradient boosting model.
...
The data is split using 75% for training and 25% for test. Initially, the split was 80% training, but this caused the model to predict $Loan\_Status$ as Yes for every instance. By decreasing the training percentage, the model performed better.
...
lap_data_split <- initial_split(lap_data, prop=0.75, strata=Loan_Status)

lap_training <- lap_data_split %>% training()

lap_test <- lap_data_split %>% testing()

lap_folds <- vfold_cv(lap_training, v=5, repeats=3)
```

## Model definition

The $xgboost$ model engine creates the gradient boosting model. The tree input parameter is set to 100 based on trial and error. Blog post set the trees value to 1000, but sample runs above 100 did not improve accuracy. For the additional parameters, the tune() method allows the parameters to be determined by a tune grid to identify the best performing parameter definitions.

```{r warning=F,message=F}
# Specify the model
mod_boost <- boost_tree(trees = 100,
                        learn_rate = tune(),
                        sample_size = tune(),
                        mtry = tune(),
                        min_n = tune(),
                        tree_depth = tune()) %>%
  set_engine("xgboost", lambda=0, alpha=1, num_class=3, verbose=1) %>%
  set_mode("classification")

```
The parameters are defined by the sampling size and then the parameter grid is built by the function grid_max_entropy(). The size of 10 was selected based on trial and error. Values above 10 did not provide improved model accuracy.
...
Following the tidymodels approach to model creation, the workflow is defined for the gradient boosting model.
...
The tune_grid() method computes the evaluation metrics based on the parameters of the model. The results with the highest Kappa value are displayed along with a Kappa value plotted against each parameter value.
...
boost_tuning %>% collect_metrics() %>% 
  ...
  labs(y="Kappa", x = "Parameter")
```

Based on the highest Kappa value, the best gradient boosting model is selected.
...
With the best model selected, the workflow is finalized before the model is fit based on the training data and subsequently generating predictions on the test data from the $lap\_data\_split$.
...
The model produces and accuracy of 85% and an AUC of 82.9% based on the fit metrics. The ROC-AUC is plotted to confirm the calculated percentage.
...
The predictions from the gradient boosting model product a final accuracy of 85% and Kappa of 60.7% with the final confusion matrix of the test data predictions.
