...
```
```{r, echo=F}
vsurfStages <- tibble('Stage' = "Final" , 'Fields' =colnames(use_data%>%dplyr::select(-Loan_Status))[surfedModel$varselect.pred])
vsurfStages <-vsurfStages %>% add_row('Stage' = '2nd' , 'Fields'=paste(colnames(use_data%>%dplyr::select(-Loan_Status))[surfedModel$varselect.interp], collapse=", "))
vsurfStages <-vsurfStages %>% add_row('Stage' = '3rd' , 'Fields'=paste(colnames(use_data%>%dplyr::select(-Loan_Status))[surfedModel$varselect.thres], collapse = ", "))
vsurfStages
...
kable(actual_result)
```
Looking at breakdowns of performance we see the best performance using Kappa to be mtry of 3 and trees of 2000. Unfortunately this doesn't 


Our full model has an accuracy of 0.811 and a Kappa of 0.455  Unfortunately, this is less than our "Model" of a simple credit_history. 
