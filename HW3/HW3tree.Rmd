...
```{r}
GGally::ggpairs(cla, mapping = "Loan_Status", columns = c(6:8, 13, 12)) %>% print()
```
...
```{r}
GGally::ggpairs(cla, mapping = "Loan_Status", columns = c(1:5, 10:12), ) %>% print()
```
...
```{r}
set.seed(1234)
loan_split <- initial_split(cla, prop = .8)
